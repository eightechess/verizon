...
public class BeaconDataToRadarSystemServiceImpl implements BeaconDataToRadarSystemService {
        ...
        return CompletableFuture.supplyAsync(() -> {
            ...
            while(true) {
                try {
                    ...
                    Throwable rootCause = unwrapThrowable(e);
                    if (!isRetryableException(rootCause)) {
                        logger.error("BeaconDataToRadarSystemService | withRetry | non-retryable failure {}. Failing fast.",
                                formatRetryError(rootCause));
                    ...
                    logger.warn("BeaconDataToRadarSystemService | withRetry | attempt:{}/{} failed with {}. Retrying after {} ms",
                            attempt,
                            maxAttempts,
                            formatRetryError(rootCause),
                            sleepDelay);
    ...
    public void mapBeaconDataAndSendToRadarSystem(Map<String, List<MerakiResponse>> stringListMap) {
        ...
        Map<String, RadarResponseData> radarCacheByApiKey = new HashMap<>();
        Set<String> deletedRadarIds = new HashSet<>();

        if (stringListMap == null || stringListMap.isEmpty()) {
            logger.info("BeaconDataToRadarSystemService | mapBeaconDataAndSendToRadarSystem | No grouped beacon data received for Radar processing");
            return;
        }
        ...
        stringListMap.forEach((division, entries) -> {
            ...
            if (StringUtils.isBlank(apiKey) || CollectionUtils.isEmpty(entries)) {
                logger.info("BeaconDataToRadarSystemService | mapBeaconDataAndSendToRadarSystem | Skipping division:{} due to blank apiKey or empty entries",
                        StringUtils.defaultIfBlank(division, BeaconRadarConstants.UNKNOWN));
                return;
            }
        ...
        entriesByApiKey.forEach((apiKey, entries) -> {
            ...
            int incomingCount = entries == null ? 0 : entries.size();
            totalIncoming.addAndGet(incomingCount);

            List<MerakiResponse> deduplicatedEntries = (entries == null ? Collections.<MerakiResponse>emptyList() : entries).stream()
                    .filter(entry -> entry != null && StringUtils.isNotBlank(entry.getTag()) && StringUtils.isNotBlank(entry.getExternalId()))
                    .collect(Collectors.toMap(
                    ...
                    .stream()
                    .collect(Collectors.toList());
            totalDeduplicated.addAndGet(deduplicatedEntries.size());

            RadarResponseData radarResponseData = radarCacheByApiKey.computeIfAbsent(apiKey, this::getRadarSystemData);
            if (radarResponseData == null || radarResponseData.getBeacons() == null) {
                logger.warn("BeaconDataToRadarSystemService | mapBeaconDataAndSendToRadarSystem | Radar response was null/empty for apiKey. Continuing with empty beacons cache");
                radarResponseData = emptyRadarResponseData();
                radarCacheByApiKey.put(apiKey, radarResponseData);
            }

            if (Boolean.TRUE.equals(appConfigProperties.getEnableRadarInvalidEntryDeletion())) {
                removeInvalidEntriesAtRadar(radarResponseData, deduplicatedEntries, apiKey, deletedRadarIds);
            } else {
                logger.info("BeaconDataToRadarSystemService | mapBeaconDataAndSendToRadarSystem | Deletion of invalid Radar entries is disabled");
            }

            ProcessingSummary summary = filterAndSendBeaconDataToRadarAndCollect(apiKey, deduplicatedEntries, radarResponseData);
        ...
        logger.info("BeaconDataToRadarSystemService | mapBeaconDataAndSendToRadarSystem | Total beacon data processed | batches:{} | incoming:{} | deduplicated:{} | duplicatesRemoved:{} | created:{} | attemptedExistingUpdates:{} | invalidInput:{} | radarDeletesAttempted:{}",
    ...
    public void filterAndSendBeaconDataToRadar(String apiKey, List<MerakiResponse> value) {
        ...
        filterAndSendBeaconDataToRadarAndCollect(apiKey, value, radarResponseData);
    }

    private ProcessingSummary filterAndSendBeaconDataToRadarAndCollect(String apiKey, List<MerakiResponse> value, RadarResponseData radarResponseData) {
    ...
    private void removeInvalidEntriesAtRadar(RadarResponseData radarResponseData,
        ...
        List<MerakiResponse> desiredBeacons = beaconResponseData == null ? Collections.emptyList() : beaconResponseData;
        Set<String> desiredTagExternalIds = desiredBeacons.stream()
                .filter(Objects::nonNull)
                .filter(b -> StringUtils.isNotBlank(b.getTag()) && StringUtils.isNotBlank(b.getExternalId()))
                .map(b -> b.getTag() + ":" + b.getExternalId())
                .collect(Collectors.toSet());

        Set<String> desiredExternalIds = desiredBeacons.stream()
                .filter(Objects::nonNull)
                .map(MerakiResponse::getExternalId)
                .filter(StringUtils::isNotBlank)
                .collect(Collectors.toSet());

        if (desiredTagExternalIds.isEmpty()) {
            logger.warn("BeaconDataToRadarSystemService | removeInvalidEntriesAtRadar | No desired beacon keys supplied for apiKey batch. Skipping deletion to avoid accidental full Radar project cleanup.");
            return;
        }
        ...
        radarResponseData.getBeacons().stream()
                ...
                .filter(entry -> !desiredTagExternalIds.contains(entry.getTag() + ":" + entry.getExternalId()))
                .filter(entry -> !desiredExternalIds.contains(entry.getExternalId()))
                .forEach(entry -> {
                    String radarId = entry.get_id();
                    if (StringUtils.isBlank(radarId)) {
                        logger.warn("BeaconDataToRadarSystemService | removeInvalidEntriesAtRadar | Skipping delete because Radar id is blank | tag:{} | externalId:{}",
                                entry.getTag(),
                                entry.getExternalId());
                        return;
                    }
                    if (deletedRadarIds != null && !deletedRadarIds.add(radarId)) {
                        logger.info("BeaconDataToRadarSystemService | removeInvalidEntriesAtRadar | Skipping duplicate delete for radarId:{}",
                                radarId);
                        return;
                    }
                    deleteAttemptCount.incrementAndGet();
                    removeEntryAtRadar(radarId, apiKey);
                });

        logger.info("BeaconDataToRadarSystemService | removeInvalidEntriesAtRadar | Summary | desiredKeys:{} | desiredExternalIds:{} | radarCacheCount:{} | deleteAttempts:{}",
                desiredTagExternalIds.size(),
                desiredExternalIds.size(),
                radarResponseData.getBeacons().size(),
                deleteAttemptCount.get());
    }
